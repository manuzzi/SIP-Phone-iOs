...
**Fix successivo (durante il test sul campo di M5): nome contatto assente in chiamata.** L'utente notava solo il numero grezzo, mai il nome del contatto — nella scheda "Telefono" compariva l'URI SIP completo (`sip:3482556548@93.70.98.172`), non il numero nudo. Causa: `CallManager` costruiva sempre un `CXHandle` di tipo `.generic`, spesso con l'URI SIP intero come valore. CallKit abbina automaticamente un contatto in rubrica (schermata di chiamata di sistema, Recenti) **solo** con `CXHandle(type: .phoneNumber, value: <numero nudo>)` — mai con `.generic`, mai con un URI completo. Corretto in due punti indipendenti: (1) `CallManager.phoneStyleHandle(_:)` sceglie `.phoneNumber` quando il valore è puramente numerico, usato ovunque venga costruito un `CXHandle`; (2) nuovo `ContactResolver` (framework Contacts, richiede `NSContactsUsageDescription` + permesso utente) per risolvere il nome anche nella UI *propria* dell'app (che non beneficia dell'abbinamento automatico di CallKit), usato sia per `remoteDisplayName` che per il nome mostrato nelle chiamate innescate da push.

### M5 — Robustezza e uso quotidiano 🔄 in corso (test sul campo in attesa)
- Gestione cambio rete (WiFi casa ↔ cellulare+VPN ↔ perdita connessione) con ri-registrazione automatica, con **`ServerReachabilityProbe`** (connessione TCP di prova sulla porta 5060) che verifica la raggiungibilità reale del server prima di forzare la ri-registrazione — vedi "Percorso reale" più sotto per come ci si è arrivati. Validato su dispositivo reale sia per WiFi→VPN sia VPN→WiFi, incluse transizioni con più cambi di percorso ravvicinati
...
- `DebugFileLogger`: log di debug persistito su file nella sandbox dell'app (`Documents/debug.log`, recuperabile via `devicectl device copy from`), aggiunto perché la cattura live della console via devicectl si è dimostrata troppo fragile per sopravvivere a un cambio di rete reale — utile anche oltre questa diagnosi specifica per il test sul campo
